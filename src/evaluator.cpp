...
void Evaluator::computeOverRepSeq(string filename, map<string, long> &hotseqs, int seqlen) {
    ...
    iter = hot_s.begin();
    while (iter != hot_s.end()) {
        string seq = iter->first;
        long count = iter->second;
        bool isSubString = false;
        for (iter2 = hot_s.begin(); iter2 != hot_s.end(); iter2++) {
    ...
    std::cout << hotseqs.size() << std::endl;

    // output for test
//    for (iter = hotseqs.begin(); iter != hotseqs.end(); iter++) {
//        cerr << iter->first << ": " << iter->first.size() << " , " << iter->second << endl;
//    }
    ofstream ofs;
